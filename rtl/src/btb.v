...
module btb #(
  ...
  output        btb_brpred_raspush,
  output        btb_brpred_raspop,
  ...
  localparam ATTR_UNCOND = 0;
  localparam ATTR_RASPUSH = 1;
  localparam ATTR_RASPOP = 2;
  localparam ATTR_WIDTH = 3;

  localparam WAY_WIDTH = 1 + TAG_WIDTH + TGT_WIDTH + ATTR_WIDTH;
  localparam SET_WIDTH = (BTB_NUM_WAYS * WAY_WIDTH) + LRU_WIDTH;

  wire                               alloc_valid;
  wire [ATTR_WIDTH-1:0]              alloc_attr;
  wire [31:2]                        alloc_target;
  wire                               alloc_valid_r;
  wire [ATTR_WIDTH-1:0]              alloc_attr_r;
  ...
  wire [BTB_NUM_WAYS*ATTR_WIDTH-1:0] rd_way_attr;
  wire [BTB_NUM_WAYS-1:0]            rd_way_uncond;
  wire [BTB_NUM_WAYS-1:0]            rd_way_raspush;
  wire [BTB_NUM_WAYS-1:0]            rd_way_raspop;
  ...
  assign alloc_attr[ATTR_UNCOND] = rob_ret_uncond | rob_ret_bptaken;
  assign alloc_attr[ATTR_RASPUSH] = rob_ret_raspush;
  assign alloc_attr[ATTR_RASPOP] = rob_ret_raspop;
  assign alloc_target = rob_ret_target;

  dffr               u_alloc_valid_r  (alloc_valid_r,  alloc_valid,  clk, 1'b1, rst);
  dff  #(ATTR_WIDTH) u_alloc_attr_r   (alloc_attr_r,   alloc_attr,   clk, alloc_valid);
  dff  #(30)         u_alloc_target_r (alloc_target_r, alloc_target, clk, alloc_valid);

  assign alloc_way_wr_data = {alloc_target_r,rd_tag_r,alloc_attr_r,1'b1};
...
`endif
  ...
  generate
    for(i = 0; i < BTB_NUM_WAYS; i=i+1) begin
      ...
      assign rd_way_uncond[i] = rd_way_attr[i*ATTR_WIDTH+ATTR_UNCOND];
      assign rd_way_raspush[i] = rd_way_attr[i*ATTR_WIDTH+ATTR_RASPUSH];
      assign rd_way_raspop[i] = rd_way_attr[i*ATTR_WIDTH+ATTR_RASPOP];
  ...
  function [WAY_WIDTH-1:0] uncond_update_way_wr_data(
    input [WAY_WIDTH-1:0] rd_data,
    input                 hit_way);

    begin
      uncond_update_way_wr_data = rd_data;
      if(hit_way)
        uncond_update_way_wr_data[1+ATTR_UNCOND] = 1'b0;
    end
  endfunction
  ...
  generate
    for(j = 0; j < BTB_NUM_WAYS; j=j+1) begin
      ...
      assign uncond_update_wr_data[j*WAY_WIDTH+:WAY_WIDTH] =
          uncond_update_way_wr_data(rd_data[j*WAY_WIDTH+:WAY_WIDTH], hit_way[j]);
    end
  endgenerate
  ...
  premux #(1,BTB_NUM_WAYS) u_btb_brpred_raspush (
    .sel(hit_way),
    .in(rd_way_raspush),
    .out(btb_brpred_raspush));
  premux #(1,BTB_NUM_WAYS) u_btb_brpred_raspop (
    .sel(hit_way),
    .in(rd_way_raspop),
    .out(btb_brpred_raspop));
